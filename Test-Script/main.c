...
static const char* get_conflict_json_id_empyt_conf(char* conflict,char* resolution)
{
    struct json_object *file_json = json_object_from_file(".git/rr-cache/conflict_index.json");
    if (!file_json) { // if file is empty
        if (!resolution) { //resolution is NULL
                ...
                if(total_similarity>=max_sim){
                    if(cluster_flags[i]==0 && cluster_flags[j]==0){
                        ...
                        for(int k=0;k<size;k++){
                            ...
                            const char* conflict_string = json_object_get_string(json_object_object_get(conflict, "conflict"));
                            const char* resol_string = json_object_get_string(json_object_object_get(conflict, "resolution"));
                            cluster_conflict_sim = cluster_conflict_sim+jaro_winkler_distance(jconf2,conflict_string);
                        ...
                        for(int k=0;k<size;k++){
                            ...
                            const char* conflict_string = json_object_get_string(json_object_object_get(conflict, "conflict"));
                            const char* resol_string = json_object_get_string(json_object_object_get(conflict, "resolution"));
                            cluster_conflict_sim = cluster_conflict_sim+jaro_winkler_distance(jconf1,conflict_string);
...
static int check_for_recluster(int conflict_number){
    ...
    if(intracluster_similarity<=intrasimilarity_th){ //check intrasimilatiry threshold
	    ...
	    printf("Clusters: %d - 1-elementcluster: %d - Percentage: %f - Conflicts in Cluster: %d - Diff from last: %d - change percentage: %f \n",cluster_count,clusters_lenght_1,percentage_of_singleclusters,x1,(x1-cluster_population),percentage_wrt_last);
            //if(percentage_of_singleclusters < valid_cluster_th){//Start reclustering
            if(percentage_of_singleclusters < valid_cluster_th && (percentage_wrt_last>=10 || diff_first_time>=250)/*(x1-cluster_population)>250*/){//Start reclustering
		...
		printf("Start reclustering...\n");
                ...
                int number_keys=0;
                //char ids[256];
                json_object_object_foreach(file_json_reclustered,key,val){//simple check for number of conflicts parity
                    //obj1 = NULL;
                    //jarray = json_object_new_array();
                    int arraylen = json_object_array_length(val);
                    //printf("cluster %d: %d \n",key,arraylen);
...
static void regex_repalce_suggestion(char *conflict, char *resolution,int jid,char *jv2,char *jdec)
{
	...
	if(conflict == '\0'){
	    ...
	    //jv1= escapeCSV(jv1);
		    ...
		    resolution=escapeCSV(resolution);
                    ...
                    fprintf(fp,"\"%s\",\"%s\",\"%f\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%d\"\n",conflict,groupId,jw1,regex1,replace1,resolution,res1,jv2,jdec,jid);
		    free(conflict);
                    free(regex1);
                    free(replace1);
                    free(res1);
		    ...
		    resolution=escapeCSV(resolution);
                    ...
                    fprintf(fp,"\"%s\",\"%s\",\"%f\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%d\"\n",conflict,groupId,jw2,regex2,replace2,resolution,res2,jv2,jdec,jid);
		    free(conflict);
                    free(regex2);
                    free(replace2);
                    free(res2);
        ...
        if (!status) {
            ...
            if(res1 && res2){
                ...
                if(jw1 >= jw2){
                    ...
                    //fprintf(fp,"\"%s\",\"%s\",\"%f\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%d\"\n",conflict,groupId,jw1,regex1,replace1,resolution,res1,jv1,jv2,jdec,jid);
                    //fprintf(fp,"\"%s\",\"%s\",\"%f\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%d\"\n",conflict,groupId,jw1,regex1,replace1,resolution,res1,jv2,jdec,jid);
                    fprintf(fp,"\"%s\",\"%s\",\"%f\",\"%s\",\"%s\",\"%s\",\"%s\"\n",conflict,groupId,jw1,regex1,replace1,resolution,res1);
		    free(conflict);
                    free(regex1);
                    free(replace1);
                    free(res1);
                    ...
                    //fprintf(fp,"\"%s\",\"%s\",\"%f\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%d\"\n",conflict,groupId,jw2,regex2,replace2,resolution,res2,jv1,jv2,jdec,jid);
                    //fprintf(fp,"\"%s\",\"%s\",\"%f\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%d\"\n",conflict,groupId,jw2,regex2,replace2,resolution,res2,jv2,jdec,jid);
                    fprintf(fp,"\"%s\",\"%s\",\"%f\",\"%s\",\"%s\",\"%s\",\"%s\"\n",conflict,groupId,jw2,regex2,replace2,resolution,res2);
		    free(conflict);
                    free(regex2);
                    free(replace2);
                    free(res2);
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < arraylen; i++) {
            ...
            regex_repalce_suggestion(jconf,jresol,jid,jv2,jdec);
